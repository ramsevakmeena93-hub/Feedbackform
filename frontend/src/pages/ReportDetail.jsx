          ...
          <div className="bg-white rounded-2xl shadow-sm overflow-hidden">
            <div className="flex items-center gap-3 px-6 py-4 bg-gradient-to-r from-red-50 to-rose-50 border-b border-red-100">
              ...
              <div>
                ...
                <p className="text-xs text-red-400">Student feedback summary</p>
              </div>
            ...
            <div className="p-6 space-y-4">
              ...
              {/* Percentage breakdown — from backend or calculated from existing data */}
              {(() => {
                // Use backend percentages if available, otherwise calculate from appreciation array
                let pcts = report.commentPercentages && Object.keys(report.commentPercentages).length > 0
                  ? report.commentPercentages
                  : null;

                if (!pcts) {
                  // Fallback: calculate from appreciation comments
                  const all = report.appreciation || [];
                  const counts = { 'Excellent': 0, 'Very Good': 0, 'Good': 0 };
                  all.forEach(c => {
                    const lower = c.toLowerCase().trim();
                    if (['excellent', 'outstanding', 'superb', 'brilliant', 'best'].some(k => lower.includes(k))) counts['Excellent']++;
                    else if (['very good', 'very well', 'very nice'].some(k => lower.includes(k))) counts['Very Good']++;
                    else if (['good', 'great', 'nice', 'well done'].some(k => lower.includes(k))) counts['Good']++;
                  });
                  const total = Object.values(counts).reduce((s, v) => s + v, 0);
                  if (total > 0) {
                    pcts = {};
                    Object.entries(counts).forEach(([k, v]) => { if (v > 0) pcts[k] = Math.round((v / total) * 100); });
                  }
                }

                if (!pcts || Object.keys(pcts).length === 0) return null;

                return (
                  <div className="bg-red-50 border border-red-100 rounded-xl p-4">
                    <p className="text-xs font-semibold text-red-600 uppercase tracking-wide mb-3">Comment Breakdown</p>
                    <div className="space-y-2">
                      {Object.entries(pcts).sort((a, b) => b[1] - a[1]).map(([label, pct]) => (
              ...
              {/* Long/meaningful comments */}
              {report.appreciation
                .filter(t => t.trim().split(/\s+/).length > 4)
                .map((text, i) => (
                  <div key={i} className="flex gap-3 items-start group">
                    <span className="mt-2 w-2.5 h-2.5 rounded-full bg-red-500 shrink-0"></span>
